...
- **Sender Policy Framework (SPF)**
  This method allows the detection of forged sender addresses. 
  In particular SPF information are contained inside a `TXT` DNS record. These info specify which IP addresses are allowed to send emails on the behalf of the  domain. Allowed senders can be listed explicitly specifying authorized IP ranges, or checking if `A`, `AAAA`, `MX` or `PTR` records resolve to the sender's address.
  ...
  The SPF `TXT` DNS record is composed of SPF version (with the tag `v:`) and one of more *mechanisms*.
...
- **Domain-based Message Authentication, Reporting and Conformance (DMARC)**
  ...
  It's basically an extension of DKIM and SPF, these two methods offer different measures of the authenticity of an email; with DMARC it's possible to specify how to employ DKIM and SPF (singularly or in combination), how the `Form:` field is presented to the client and how to deal with authentication failures.

  When DMARC is in place, an email needs to pass 3 alignments to be valid: SPF alignment, DKIM alignment and DMARC alignment.

  DMARC alignment basically checks that the domain name in the `Form:` field, is "aligned" with either DKIM or SPF aligned ones.
...
Sadly I couldn't experiment much at the moment, but I will for sure try and send emails from a VPS or a Virtual Machine that use a non-residential IP, I'm confident this approach would work in sending spoofed emails on vulnerable domains.  
